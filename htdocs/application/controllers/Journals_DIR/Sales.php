...
class Sales extends MY_Controller{
	...
	public function get_sales_journal(){
		return $this->load->view($this->layout,  ['head_css'=>'fragments/head_css/journals/journals', 
													...
													'transaction'=>'fragments/content/journals/parts/new_transaction',
	...
	public function transaction_vat(){
		...
		echo json_encode(Sales_Model::get_vat($this->session->userdata('user'), $bp_id));
	}
	...
	public function transaction_ewt(){
		$bp_id = $this->input->get('id');
		echo json_encode(Sales_Model::get_ewt($this->session->userdata('user'), $bp_id));
	}

	public function transaction_fwt(){
		$bp_id = $this->input->get('id');
		echo json_encode(Sales_Model::get_fwt($this->session->userdata('user'), $bp_id));
	}
	...
	public function je_account(){
		echo json_encode(Sales_Model::je_account($this->session->userdata('user')));
	}

	public function je_pcc(){
		echo json_encode(Sales_Model::je_pcc($this->session->userdata('user')));
	}

	public function je_dept(){
		echo json_encode(Sales_Model::je_dept($this->session->userdata('user')));
	}
	...
	public function save_sale_trans(){
		...
		return redirect('journals/sales', 'refresh');
	}

	// ADDITIONAL

	public function last_trans_id(){
		echo json_encode(Sales_Model::last_trans_id($this->session->userdata('user')));
	}
	public function last_journ_trans_id(){
		echo json_encode(Sales_Model::last_journ_trans_id($this->session->userdata('user'), $this->input->get('id')));
	}
