...
class Receipts extends MY_Controller{
	...
	public function get_receipts_journal(){
		return $this->load->view($this->layout,  ['head_css'=>'fragments/head_css/journals/journals', 
													...
													'content'=>'fragments/content/journals/receipts_journal', 
													'footer_js'=>'fragments/footer_js/journals/receipts_journal', 
													'back_button'=>'home', 
													...
													'transaction'=>'fragments/content/journals/parts/new_transaction_receipts',
													'document'=>'fragments/content/journals/parts/invoice',
												]);
	}

	public function get_journals_summary(){
		echo json_encode(['data' => Receipts_Model::get_journals_summary($this->session->userdata('user'))]);
	}

	public function get_bp(){
		echo json_encode(['data' => Receipts_Model::get_bp($this->session->userdata('user'))]);
	}

	public function get_bp_transaction(){
		echo json_encode(['data' => Receipts_Model::get_bp_transaction($this->input->get('bp_id'), $this->session->userdata('user'))]);
	}

	public function transaction_bp(){
		echo json_encode(Receipts_Model::get_bp($this->session->userdata('user')));
	}

	public function transaction_bank(){
		echo json_encode(Receipts_Model::get_bank($this->session->userdata('user')));
	}

	public function transaction_product_service(){
		echo json_encode(Receipts_Model::get_product_service($this->session->userdata('user')));
	}

	public function transaction_vat(){
		$bp_id = $this->input->get('id');
		echo json_encode(Receipts_Model::get_vat($this->session->userdata('user'), $bp_id));
	}

	public function transaction_discount(){
		echo json_encode(Receipts_Model::get_discount($this->session->userdata('user')));
	}

	public function transaction_ewt(){
		$bp_id = $this->input->get('id');
		echo json_encode(Receipts_Model::get_ewt($this->session->userdata('user'), $bp_id));
	}

	public function transaction_fwt(){
		$bp_id = $this->input->get('id');
		echo json_encode(Receipts_Model::get_fwt($this->session->userdata('user'), $bp_id));
	}

	public function transaction_doc_ref(){
		echo json_encode(Receipts_Model::get_doc_ref($this->session->userdata('user')));
	}

	public function transaction_mop(){
		echo json_encode(Receipts_Model::get_mop($this->session->userdata('user')));
	}

	public function je_account(){
		echo json_encode(Receipts_Model::je_account($this->session->userdata('user')));
	}

	public function je_pcc(){
		echo json_encode(Receipts_Model::je_pcc($this->session->userdata('user')));
	}

	public function je_dept(){
		echo json_encode(Receipts_Model::je_dept($this->session->userdata('user')));
	}
	...
	public function save_receipt_trans_upload(){
		$document = [];
		$products = [];
		$services = [];
		$vat = [];
	...
	public function save_receipt_trans(){
		$trans_data = [
		...
		Receipts_Model::save_trans($trans_data, $prod_data, $serv_data, $vat_data, $discount_data, $ewt_data, $fwt_data, $doc_ref_data, $bank_data, $other_details, $journal_entry_data);
		return redirect('journals/receipts', 'refresh');
	}

	// ADDITIONAL

	public function last_trans_id(){
		echo json_encode(Receipts_Model::last_trans_id($this->session->userdata('user')));
	}
	public function last_journ_trans_id(){
		echo json_encode(Receipts_Model::last_journ_trans_id($this->session->userdata('user'), $this->input->get('id')));
	}
