<script src="<?php echo base_url(); ?>/assets/js/apps/transactions/app.js"></script>
<script src="<?php echo base_url(); ?>/assets/js/apps/transactions/controllers/transaction.js"></script>
<script src="<?php echo base_url(); ?>/assets/js/apps/transactions/directives/transaction.js"></script>

<script>
	var summary_table = $('#transactions-journal-summary-table').DataTable({
							ajax: window_location+'/transactions/journals_summary/get',
							scrollX: true,
							columns: [
										{'data': 'co_trans_id'},
										{
	...
	var bp_summary_table = $('#transactions-bp-table').DataTable({
								ajax: window_location+'/transactions/bp/get',
								columns: [
											{'data': 'bpc_class'},
											{'data': 'co_bp_code'},
											{'data': 'bp_name'},
	...
	var bp_transaction_details_table = $('#transactions-transaction-details-table').DataTable({
							ajax: window_location+'/transactions/bp_transaction/get?bp_id=0',
							columns: [
										{'data': 'co_trans_id'},
										{
											mData: function(rowData, settings, fullData){
...
<script>
	$('#transactions-journal-summary-table_wrapper tbody').on('click', 'tr', function(){
	...
	$('#transactions-bp-table_wrapper tbody').on('click', 'tr', function(){
		var row_data = bp_summary_table.row(this).data();
		bp_transaction_details_table.ajax.url(window_location+'/transactions/bp_transaction/get?bp_id='+row_data.co_bp_id).load();
		$(this).closest('table').find('tr').css('background-color', '#FFF');
		$(this).closest('table').find('tr').css('color', 'rgba(0,0,0,.87)');
		$(this).css('background-color', 'rgb(133, 150, 214)');
		$(this).css('color', '#FFF');
	...
	$('#transactions-transaction-details-table_wrapper tbody').on('click', 'tr', function(){
			...
			$.each(row_data.prod_data, function(index, data){
				$('#bp-trans-prod-serv').append("<tr>"+
													"<td><input class='form-control' type='text' value='"+data.co_p_code+"' readonly></td>"+
														...
														"<td><input class='form-control' type='text' value='"+data.co_trans_prod_gross+"'  readonly></td>"+
													"</tr>");
					t_prod_serv_qty += parseFloat(data.co_trans_prod_qty);
					t_prod_serv_unit += parseFloat(data.co_trans_prod_unit);
					t_prod_serv_unit_price += parseFloat(data.co_trans_prod_unitprice);
					t_prod_serv_gross += parseFloat(data.co_trans_prod_gross);
			});
			$.each(row_data.serv_data, function(index, data){
				$('#summary-prod-serv').append("<tr>"+
													"<td><input class='form-control' type='text' value='"+data.co_s_code+"' readonly></td>"+
														...
														"<td><input class='form-control' type='text' value='"+data.co_trans_serv_gross+"'  readonly></td>"+
													"</tr>");
					t_prod_serv_qty += parseFloat(data.co_trans_serv_qty);
					t_prod_serv_unit += parseFloat(data.co_trans_serv_unit);
					t_prod_serv_unit_price += parseFloat(data.co_trans_serv_unitprice);
					t_prod_serv_gross += parseFloat(data.co_trans_serv_gross);
			});
		}
		...
		if(row_data.vat_data.length === 0){
			...
			$.each(row_data.vat_data, function(index, data){
				$('#bp-trans-vat').append("<tr>"+
								...
								"<td><input class='form-control' type='text' value='"+data.co_trans_vat_gross+"' readonly></td>"+
							"</tr>");
					t_vat_rate += parseFloat(data.t_rate);
					t_vat_amount += parseFloat(data.co_trans_vat_amount);
					t_vat_net += parseFloat(data.co_trans_vat_net_amount);
					t_vat_gross += parseFloat(data.co_trans_vat_gross);
			});
		}
		...
		if(row_data.discounts_data.length === 0){
			...
			$.each(row_data.discounts_data, function(index, data){
				$('#bp-trans-discount').append("<tr>"+
												...
												"<td><input class='form-control' type='text' value='"+data.co_trans_disc_scid+"' readonly></td>"+
											"</tr>");
					t_discount_rate += parseFloat(data.co_d_rate);
					t_discount_deduction += parseFloat(data.co_trans_disc_deduction);
			});
		}
		...
		if(row_data.ewt_data.length === 0){
			...
			$.each(row_data.ewt_data, function(index, data){
				$('#bp-trans-ewt').append("<tr>"+
											...
											"<td><input class='form-control' type='text' value='"+data.co_trans_ewt_taxwithheld+"' readonly></td>"+
										"</tr>");
					t_ewt_rate += parseFloat(data.t_rate);
					t_ewt_tax_base += parseFloat(data.t_base);
					t_ewt_tax_withheld += parseFloat(data.co_trans_ewt_taxwithheld);
			});
		}
		...
		if(row_data.fwt_data.length === 0){
			...
			$.each(row_data.fwt_data, function(index, data){
				$('#bp-trans-fwt').append("<tr>"+
											...
											"<td><input class='form-control' type='text' value='"+data.co_trans_fwt_taxwithheld+"' readonly></td>"+
										"</tr>");
					t_fwt_rate += parseFloat(data.t_rate);
					t_fwt_tax_base += parseFloat(data.t_base);
					t_fwt_tax_withheld += parseFloat(data.co_trans_fwt_taxwithheld);
			});
		}
		...
		if(row_data.doc_reference_data.length === 0){
			...
			$.each(row_data.doc_reference_data, function(index, data){
				$('#bp-trans-doc-ref').append("<tr>"+
												"<td><input class='form-control' type='text' value='"+data.co_doc_code+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.co_trans_doc_ref_no+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.co_trans_doc_docdate+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.co_trans_doc_gross+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.co_trans_doc_netamount+"' readonly></td>"+
		...
		if(row_data.bank_details_data.length === 0){
			...
			$.each(row_data.bank_details_data, function(index, data){
				$('#bp-trans-bank').append("<tr>"+
												"<td><input class='form-control' type='text' value='"+data.co_b_code+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.b_shortname+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.co_b_no+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.co_trans_bank_det_doc+"' readonly></td>"+
												"<td><input class='form-control' type='text' value='"+data.co_trans_bank_det_amount+"' readonly></td>"+
		...
		bp_doc.find('input[name=transaction_id]').val(row_data.co_trans_id);
		bp_doc.find('input[name=transaction_date]').val(new Date(row_data.co_trans_date).toString('MMM d, yyyy'));
		bp_doc.find('input[name=journal_trans_id]').val(row_data.co_journ_trans_id);
		bp_doc.find('input[name=company_name]').val(row_data.cb_name);
		bp_doc.find('input[name=document_name]').val(row_data.co_trans_doc_name);
		bp_doc.find('input[name=company_address]').val(row_data.cb_address);
		bp_doc.find('input[name=document_number]').val(row_data.co_trans_doc_no);
		bp_doc.find('input[name=company_tin]').val(row_data.cb_tin);
		bp_doc.find('input[name=document_date]').val(new Date(row_data.co_trans_doc_date).toString('MMM d, yyyy'));
		bp_doc.find('input[name=bp_name]').val(row_data.bp_name);
		bp_doc.find('input[name=payment_type]').val(row_data.co_trans_doc_pay_type);
		bp_doc.find('input[name=bp_address]').val(row_data.co_bp_address);
		bp_doc.find('input[name=terms]').val(row_data.co_trans_doc_pay_terms);
		bp_doc.find('input[name=tin]').val(row_data.co_bp_tin);
		bp_doc.find('input[name=due_date]').val(new Date(row_data.co_trans_doc_pay_duedate).toString('MMMM d, yyyy'));
		bp_doc.find('input[name=bp_type]').val(row_data.bpt_type);
		bp_doc.find('input[name=particulars]').val(row_data.co_bp_particulars);
		bp_doc.find('input[name=payment_mode]').val(row_data.co_mop_name);
		bp_doc.find('input[name=payment_amount]').val(row_data.co_trans_doc_pay_amountpaid);
		bp_doc.find('input[name=check_number]').val(row_data.co_trans_doc_pay_checknumber);
		bp_doc.find('input[name=check_date]').val(new Date(row_data.co_trans_date).toString('MMM d, yyyy'));
		bp_doc.find('input[name=check_payee]').val(row_data.cb_name);
		bp_doc.find('input[name=bank]').val(row_data.b_name);
		bp_doc.find('input[name=account_number]').val(row_data.co_b_no);
		...
		$('#bp-document .document_temp').removeClass('hide');
		$('#bp-document .document_1').removeClass('hide');
		$('#bp-document .document_2').removeClass('hide');
		$('#bp-document .document_3').removeClass('hide');
		if(row_data.co_journ_trans_id === '5'){
			$('#bp-document .document_temp').removeClass('hide');
			$('#bp-document .document_1').addClass('hide');
			$('#bp-document .document_3').addClass('hide');
		}else if(row_data.co_journ_trans_id === '7'){
			$('#bp-document .document_temp').removeClass('hide');
			$('#bp-document .document_1').addClass('hide');
			$('#bp-document .document_2').addClass('hide');
		}else{
			$('#bp-document .document_temp').removeClass('hide');
			$('#bp-document .document_2').addClass('hide');
			$('#bp-document .document_3').addClass('hide');
