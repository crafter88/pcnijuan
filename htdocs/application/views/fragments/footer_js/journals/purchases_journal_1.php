...
<script>
	var summary_table = $('#purchases-journal-summary-table').DataTable({
							ajax: window.location.origin+'/journals/purchases/journals_summary/get',
							columns: [
										{'data': 'co_trans_id'},
										{
	...
	var bp_summary_table = $('#purchases-bp-table').DataTable({
								ajax: window.location.origin+'/journals/purchases/bp/get',
								columns: [
											{'data': 'bpc_class'},
											{'data': 'co_bp_code'},
											{'data': 'bp_name'},
	...
	var bp_transaction_details_table = $('#purchases-transaction-details-table').DataTable({
							ajax: window.location.origin+'/journals/purchases/bp_transaction/get?bp_id=0',
							columns: [
										{'data': 'co_trans_id'},
										{
											mData: function(rowData, settings, fullData){
...
<script>
	$('#purchases-journal-summary-table_wrapper tbody').on('click', 'tr', function(){
	...
	$('#purchases-bp-table_wrapper tbody').on('click', 'tr', function(){
		var row_data = bp_summary_table.row(this).data();
		bp_transaction_details_table.ajax.url(window.location.origin+'/journals/purchases/bp_transaction/get?bp_id='+row_data.co_bp_id).load();
		$(this).closest('table').find('tr').css('background-color', '#FFF');
		$(this).closest('table').find('tr').css('color', 'rgba(0,0,0,.87)');
		$(this).css('background-color', 'rgb(133, 150, 214)');
		$(this).css('color', '#FFF');
	...
	$('#purchases-transaction-details-table_wrapper tbody').on('click', 'tr', function(){
