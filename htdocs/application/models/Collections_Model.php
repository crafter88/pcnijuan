...
class Collections_Model extends CI_Model{
	...
	public static function get_journals_summary($user){
		$co_transaction = self::$db->from('co_transaction ct')->join('journals j', 'j.j_code = ct.co_journ_trans_id')->join('co_journals cj', 'cj.j_id=j.j_id')->join('co_business_partners cbp', 'cbp.co_bp_id = ct.co_trans_doc_bp_id')->join('business_partners_type bpt', 'bpt.bpt_id = cbp.bpt_id')->join('company_history ch', 'ch.ch_cb_id = ct.cb_id')->where(['ct.cb_id' => $user->main_company->cb_id, 'ct.co_journ_trans_id' => '4', 'cj.cb_id' => $user->main_company->cb_id, 'ch.flag' => '1'])->get()->result();
