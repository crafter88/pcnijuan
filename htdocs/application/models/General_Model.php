...
class General_Model extends CI_Model{
	...
	public static function get_journals_summary($user){
		$co_transaction = self::$db->from('co_transaction ct')->join('journals j', 'j.j_code = ct.co_journ_trans_id')->join('co_journals cj', 'cj.j_id=j.j_id')->join('company_history ch', 'ch.ch_cb_id = ct.cb_id')->where(['ct.cb_id' => $user->main_company->cb_id, 'ct.co_journ_trans_id' => '6', 'cj.cb_id' => $user->main_company->cb_id, 'ch.flag' => '1'])->get()->result();
	...
	public static function get_mop($user){
		return self::$db->from('modes_of_payment mop')->join('co_modes_of_payment cmop', 'cmop.mop_id = mop.mop_id')->where(['cmop.cb_id' => $user->main_company->cb_id, 'cmop.flag' => '1'])->get()->result();
	}
