angular.module('journals')
		...
		//Document
			...
			$http.get(window_location+'/journals/receipts/transaction/last_trans_id').then(function success(response){
			...
			$http.get(window_location+'/journals/receipts/transaction/last_journ_trans_id', {params: {id: $scope.journal_transaction_id}}).then(function success(response){
				$scope.journal_transaction_count = parseFloat(response.data) + 1;
			}, function error(response){
				console.log(response);
			});
			
			$scope.document_name = 'Official Receipt';
			
		//Business Partner
			$scope.selected_bp = {};
			$scope.business_partner_array = [];
			$http.get(window_location+'/journals/receipts/transaction/bp/get').then(function success(response){
				$.each(response.data, function(index, data){
		...
		//MODE OF PAYMENT
			...
			$http.get(window_location+'/journals/receipts/transaction/mop/get').then(function success(response){
				$.each(response.data, function(index, data){
