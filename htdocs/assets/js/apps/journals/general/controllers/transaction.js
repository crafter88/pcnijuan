angular.module('journals')
		...
		//Document
			...
			$http.get(window_location+'/journals/general/transaction/last_trans_id').then(function success(response){
				$scope.transaction_id = parseFloat(response.data) + 1;
			}, function error(response){
				console.log(response);
			});
			
			$scope.journal_transaction_id = 6;
			$scope.journal_transaction_count = 1;
			$http.get(window_location+'/journals/general/transaction/last_journ_trans_id', {params: {id: $scope.journal_transaction_id}}).then(function success(response){
				$scope.journal_transaction_count = parseFloat(response.data) + 1;
			}, function error(response){
				console.log(response);
			});
			
			$scope.document_name = 'Journal Voucher';
			
		//Business Partner
			$scope.selected_bp = {};
			$scope.business_partner_array = [];
			$http.get(window_location+'/journals/general/transaction/bp/get').then(function success(response){
				$.each(response.data, function(index, data){
		...
		//MODE OF PAYMENT
			...
			$http.get(window_location+'/journals/general/transaction/mop/get').then(function success(response){
				$.each(response.data, function(index, data){
