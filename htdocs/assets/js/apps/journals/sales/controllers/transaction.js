angular.module('journals')
		...
		//Document
			...
			$http.get(window_location+'/journals/sales/transaction/last_trans_id').then(function success(response){
			...
			$http.get(window_location+'/journals/sales/transaction/last_journ_trans_id', {params: {id: $scope.journal_transaction_id}}).then(function success(response){
				$scope.journal_transaction_count = parseFloat(response.data) + 1;
			}, function error(response){
				console.log(response);
			});
			
			$scope.document_name = 'Sales Invoice';
