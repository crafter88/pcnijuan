...
var init_filter = function(table){
    ...
    filter1 = $('#filter1').selectize({
        ...
        onChange: function(value){
            ...
            table.ajax.url(window_location+'/company_settings/banks/filter_table?filter1='+val1+'&'+'filter2='+val2).load();
            init_filter2();
        }
    });
    ...
    var init_filter1 = function(){
        $.get(window_location+'/company_settings/banks/get_filter1?filter2='+$('#filter2').val(), function(response){
    ...
    var init_filter2 = function(){
        $.get(window_location+'/company_settings/banks/get_filter2?filter1='+$('#filter1').val(), function(response){
            var json_data = JSON.parse(response);
            var options = [];
            ...
            $.each(json_data, function(index, data){
                options.push({
                    text: data.co_b_class,
                    value: data.co_b_class
                });
                if(data.co_b_class === $('#filter2').val()){
                    flag = 0;
                }
